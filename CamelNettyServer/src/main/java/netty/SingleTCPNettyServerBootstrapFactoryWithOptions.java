...
public class SingleTCPNettyServerBootstrapFactoryWithOptions extends ServiceSupport implements NettyServerBootstrapFactory, Suspendable {
    ...
    public SingleTCPNettyServerBootstrapFactoryWithOptions() {
        ...
        if (options != null) {
            for (Map.Entry<String, Object> entry : options.entrySet()) {
                ...
                if (key.startsWith(OPTION_PREFIX)) {
                    key = value.substring(OPTION_PREFIX.length());
                    option = ChannelOption.valueOf(key);
                    if (key.startsWith(CHILD_PREFIX)) {
                        key = value.substring(CHILD_PREFIX.length());
                        option = ChannelOption.valueOf(key);
                    }
